# Use the 1-5000 training images to train the model (with Center-Surround in U-Net)
# Use MIT1003 dataset to finetune the pretrained model (pretrained on SALICON)
'''
...
# iters = 200*400 + 2 # taken from pix2pix paper 5.2
# iters = 200*100 + 2 # taken from pix2pix paper 5.2
iters = 5*10 + 2 # taken from pix2pix paper 5.2
# iters = 500 + 2 # taken from pix2pix paper 5.2
# iters = 200*100 + 2 # taken from pix2pix paper 5.2
batch_size = 1 # taken from pix2pix paper 5.2
...
A = np.load('train_dataset_y_MIT1003.npy') # original images
B = np.load('train_dataset_x_MIT1003.npy') # saliency
# A = np.load('val_dataset_y_1.npy') # original images
# B = np.load('val_dataset_x_1.npy') # saliency
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
    ...
    saver.restore(sess, "/media/ubuntu/CZHhy/GAN/BorjiGAN/Code/models/model_l1_Adv_3/model.ckpt") # trained on 1-10000 SALICON images for 60000 steps
    ...
    for step in range(iters):
        ...
        if ((step % 50 == 0) and (step != 0)):
            ...
            # save_path = saver.save(sess, "models/model_inverse.ckpt")
            # save_path = saver.save(sess, "models/model_mse/model.ckpt")
            save_path = saver.save(sess, "models/model_l1_Adv_4/model.ckpt")
            print("Model saved in file: %s" % save_path)
