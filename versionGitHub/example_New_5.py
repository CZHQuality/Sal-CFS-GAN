...
# The second training stage, divide SALICON training set into subsets containing 2000 images 
# This code is used to train the model (UNet+ResBlock+ L1+KLD+NSS+CC+Hist) from stratch, but the weights of NSS is high
...
A = np.load(Dataset_Root_Path + '1_dataset_y_SALICON_train.npy') # original images XXXXXXXXXXXXXXXXXXXX 
B = np.load(Dataset_Root_Path + '1_dataset_x_SALICON_train.npy') # saliency maps XXXXXXXXXXXXXXX
C = np.load(Dataset_Root_Path + '1_dataset_Pts_SALICON_train.npy') # fixation points maps XXXXXXXXXXXXXX
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, log_device_placement=True)) as sess:
    ...
    saver.restore(sess, root_path + "models/model_l1_Adv_10/model_middle.ckpt") # for finetune, not train from stratch
    ...
    # saver.restore(sess, root_path + "models/model_l1_Adv_9/model_middle.ckpt") # for finetune, not train from stratch
    # saver.restore(sess, root_path + "models/model_l1_Adv_8/model.ckpt") # for finetune, not train from stratch
    ...
    for step in range(iters):
        ...
        if ((step % 5000 == 0) and (step != 0)):
            ...
            # save_path = saver.save(sess, root_path + "models/model_l1_Adv_9/model_middle.ckpt") 
            save_path = saver.save(sess, root_path + "models/model_l1_Adv_10/model_middle.ckpt") 
