...
class GlobalUNet_old(nn.Module):
...
class GlobalUNet(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64):
        ...
        '''
        self.conv8_1 = nn.ConvTranspose2d(in_channels=256, out_channels=128, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.relu8_1 = nn.ReLU(inplace=True)
        self.norm8 = nn.InstanceNorm2d(3, affine=False)
        
        self.conv8_2 = nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.relu8_2 = nn.ReLU(inplace=True)
        
        self.conv8_3 = nn.ConvTranspose2d(in_channels=64, out_channels=3, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.relu8_3 = nn.ReLU(inplace=True)
        '''
        self.conv9_1 = nn.Conv2d(3, 3, kernel_size=3, stride=1, padding=1)
        self.tanh9_1 = nn.Tanh()
        
        self.upsp = nn.Upsample(scale_factor=2, mode='bilinear')
        ...
        self.norm6_h = nn.InstanceNorm2d(128, affine=False)
        
        self.conv7_1_h = nn.ConvTranspose2d(in_channels=128, out_channels=3, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.relu7_1_h = nn.ReLU(inplace=True)
        self.norm7_h = nn.InstanceNorm2d(3, affine=False)
    ...
    def forward(self, input):
        ...
        # d4 = nn.functional.interpolate(size=(480, 640), mode='bilinear')
        # d4 = self.upsp(d3)
        ...
        d3_h = self.conv9_1_h(d2_h)
        d3_h = self.tanh9_1_h(d3_h)
        
        # d3_upsp = self.upsp(d3)
        
        final_sm = torch.cat([d3_upsp, d3_h], dim=1)
        final_sm = self.
