...
class AlignedDataset(BaseDataset):
    def initialize(self, opt):
        ...
        if opt.isTrain or opt.use_encoded_image:
            # dir_B = 'maps/train/' if self.opt.label_nc == 0 else '_img'
    ...
    def __getitem__(self, index):        
        ...
        if self.opt.isTrain or self.opt.use_encoded_image:
            ...
            # B = B.resize((80, 64), Image.BILINEAR) # for SALICON only  
            ...
            # B = B.resize((640, 512), Image.BILINEAR) # for SALICON only , the small size should be 64 so that the smallest dimention of feature is even number, for better feature alignment
            B = B.resize((640, 480), Image.BILINEAR) 
            # B = B.resize((160, 120), Image.BILINEAR) # for SALICON only , the small size should be 64 so that the smallest dimention of feature is even number, for better feature alignment
            transform_B = get_transform(self.opt, params)      
            B_tensor = transform_B(B)

            C_path = self.C_paths[index]   
            C = Image.open(C_path).convert('RGB')
            # C = C.resize((80, 64), Image.BILINEAR) # for SALICON only  
            # C = C.resize((160, 128), Image.BILINEAR) # for SALICON only  
            # C = C.resize((640, 512), Image.BILINEAR) # for SALICON only  
            C = C.resize((640, 480), Image.BILINEAR) # for SALICON only  
            # C = C.resize((160, 120), Image.BILINEAR) # for SALICON only  
